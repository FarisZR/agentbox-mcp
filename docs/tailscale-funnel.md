...
Use this ChatGPT connector URL for the default config:

```text
https://<hostname>/mcp
```

For the simple no-OAuth ChatGPT setup, use the full secret URL generated by:

```bash
./scripts/create-chatgpt-simple-config.sh agentbox-mcp.chatgpt.toml https://<hostname>
```

It will look like:

```text
https://<hostname>/mcp/<64-hex-secret>
```
